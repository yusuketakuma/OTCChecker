...
export default function InventoryDetailPage() {
  ...
  const manualSaleTargetQuantity = totalActiveQuantity;
  ...
  const saleRemainingQuantity =
    parsedSaleQuantity === null
      ? manualSaleTargetQuantity
      : Math.max(manualSaleTargetQuantity - parsedSaleQuantity, 0);
  const saleExceedsStock =
    parsedSaleQuantity !== null && parsedSaleQuantity > manualSaleTargetQuantity;
  ...
  async function recordManualSale() {
    ...
    if (saleExceedsStock) {
      setError(`手動売上対象在庫 ${manualSaleTargetQuantity} 個を超えるため売上登録できません。`);
      setMessage("");
      return;
    }
  ...
  return (
    <div className="space-y-6">
      ...
      <Card className="space-y-4">
        <div className="grid gap-3 sm:grid-cols-2 xl:grid-cols-4">
          <div className="rounded-2xl bg-slate-50/90 p-4">
            ...
            <p className="mt-1 text-sm text-slate-500">手動売上対象 {manualSaleTargetQuantity}個</p>
          </div>
      ...
      <Card className="scroll-mt-24 space-y-4" id="manual-sale">
        ...
        <CardDescription>CSV を待たずに、その場の販売や補正売上を ACTIVE ロット全体へ FIFO で反映します。</CardDescription>
        <div className="rounded-2xl bg-slate-50/90 p-3 text-sm text-slate-700">
          <p>現在の手動売上対象在庫: {manualSaleTargetQuantity}個</p>
          <p>今回の売上後の見込み在庫: {saleRemainingQuantity}個</p>
          {saleExceedsStock ? (
            <p className="mt-2 text-[var(--color-danger)]">
              手動売上対象在庫を超える数量です。数量を減らしてから登録してください。
            </p>
          ) : null}
        </div>
        ...
        <div className="grid gap-3 sm:grid-cols-[1fr_140px]">
          ...
          <Button
            className="w-full sm:col-span-2"
            disabled={!isOnline || !saleDate || selling || parsedSaleQuantity === null || manualSaleTargetQuantity === 0 || saleExceedsStock}
            variant="secondary"
            onClick={recordManualSale}
          >
            {selling ? "登録中..." : manualSaleTargetQuantity === 0 ? "対象在庫がないため登録不可" : "売上登録"}
          </Button>
        </div>
      </Card>
