...
test("未割当の一括入荷条件はページ再訪でも保持でき、クリアできる", async ({ page }) => {
  ...
  await page.getByRole("button", { name: "5個" }).click();
...
test("一括入力後も未割当の編集中ドラフトが消えない", async ({ page }) => {
  ...
  await page.getByPlaceholder("商品名・JAN・理由で検索").fill("CSV一括入力E2E商品");
  ...
  const noteInputs = page.getByLabel(/CSV一括入力E2E商品.*の解決メモ/);
...
test("未割当行は必須入力がそろうまで解決アクションを実行できない", async ({ page }) => {
  ...
  const noteInput = unresolvedCard.getByLabel("CSV新規E2E商品の解決メモ");
