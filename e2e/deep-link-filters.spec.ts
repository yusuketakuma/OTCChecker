...
test("在庫一覧の deep link が初回表示でも検索語とバケットを反映する", async ({ page }) => {
  ...
  await expect(page.getByLabel("在庫を検索")).toHaveValue("7日以内E2E");
  await expect(page.getByLabel("在庫を検索")).toHaveAttribute("placeholder", "商品名・JANコード");
...
test("商品管理の deep link が初回表示でも検索語を反映する", async ({ page }) => {
  ...
  await expect(page.getByLabel("商品を検索")).toHaveValue("在庫なしE2E");
  await expect(page.getByLabel("商品を検索")).toHaveAttribute("placeholder", "商品名・JANコード");
