...
test("在庫詳細で商品更新・入荷・売上・ロット操作・履歴確認ができる", async ({ page }) => {
  ...
  await page.getByRole("heading", { name: "在庫操作E2E商品" }).click();
...
test("期限切れ在庫も手動売上で FIFO 対象として扱える", async ({ page, request }) => {
  const inventoryResponse = await request.get(`/api/products?q=${seededProducts.expired.janCode}`);
  expect(inventoryResponse.ok()).toBeTruthy();
  ...
  const targetProduct = inventoryItems.find((item) => item.janCode === seededProducts.expired.janCode);

  if (!targetProduct) {
    throw new Error("seeded expired product not found");
  }

  await page.goto(`/inventory/${targetProduct.productId}#manual-sale`);

  const saleSection = page.locator("#manual-sale");
  await expect(page.getByRole("heading", { name: "期限切れE2E商品" })).toBeVisible();
  await expect(saleSection.getByText("現在の手動売上対象在庫: 5個")).toBeVisible();
  await expect(saleSection.getByText("FIFO消し込み予定")).toBeVisible();
  await expect(saleSection.getByText("売上前 5個 → 売上後 4個")).toBeVisible();

  await saleSection.locator('input[type="number"]').fill("1");
  await saleSection.getByRole("button", { name: "売上登録" }).click();

  await expect(page.getByText("手動売上を登録しました。")).toBeVisible();
  await expect(page.getByText("初回 5個 / 現在 4個")).toBeVisible();
});
