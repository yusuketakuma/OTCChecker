...
test("期限切れ在庫は手動売上の対象外として扱われる", async ({ page, request }) => {
  ...
  await expect(saleSection.getByText("現在の販売可能在庫: 0個")).toBeVisible();
  await expect(
    saleSection.getByText("期限切れ在庫 5 個は手動売上の対象外です。先に廃棄登録してください。"),
  ).toBeVisible();
  await expect(
    saleSection.getByRole("button", { name: "販売可能在庫がないため登録不可" }),
  ).toBeDisabled();
});
