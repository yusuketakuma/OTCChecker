...
test("在庫詳細で商品更新・入荷・売上・ロット操作・履歴確認ができる", async ({ page }) => {
  ...
  // "4個 / 手動売上" で履歴レコードを特定
  await expect(historySection.getByText(/4個 \/ 手動売上/)).toBeVisible();

  await page.getByRole("button", { name: "廃棄", exact: true }).click();
  await expect(historySection.getByText("期限近接E2E")).toBeVisible();

  await page.getByRole("button", { name: "調整", exact: true }).click();
  await expect(historySection.getByText("棚卸差異E2E")).toBeVisible();
});
